...
- (C) 2024 DEADC0DE Edition by Tim Böttiger
...
Address ranges can be specified using the "-r" option. If the end address is omitted, the disassembly will proceed from the supplied address to the end of the file.
...
v0.95 update: FastROM is detected automatically. You can force enable/disable it by using the `-s` and `-i` options.
...
You shouldn't need this. If you do, you'll probably understand what this does anyway. Otherwise, don't worry about it.
...
Options: (numbers are hex-only, no prefixes)
 ...
 -A                Don't output addresses each line.
 -H                Don't output hex dump each line.
...
This project does not currently have an explicit license. According to copyright law, this means all rights are reserved by the original author. Please refer to the original repository for more information.
